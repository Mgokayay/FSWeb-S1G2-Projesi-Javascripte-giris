...
Aşağıdakileri yap:   
   ...
   İPUCU: fonksiyon oluşturmaya gerek yok
*/
let birinciDeger="zurafa";
let ikinciDegeri="kaplan";
if (birinciDeger.length==ikinciDegeri) {
birinciDeger="zurafa"
}
...
function sesliHarfSayaci(text) {
  const sesliHarfler = "aeıiuüoöAEIİUÜOÖ";
  let sayac = 0;

  for ( let i = 0; i < text.length; i++) {
    if ( sesliHarfler.includes(text[i])) {
      sayac++;
    }
  }
  return sayac;
}
let sesliHarfSayisi = sesliHarfSayaci("merhabaDünya");
